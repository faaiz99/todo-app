...
<template>
  ...
  <div class="flex justify-center">
    <ul>
      <li
        class="sm:flex p-2.5 gap-4 justify-center mb-2"
        v-for="task in tasks"
        :key="tasks.tasksID"
      >
        <div class="flex items-baseline">
          ...
          />
          <p
            class="ml-5 mt-1 font-semibold text-sm relative text-zinc-800"
        ...
        <p
          class="mt-1 font-normal text-sm text-zinc-800 text-left w-64 lg:w-96"
          :class="task.checked ? 'line-through' : ''"
        >
